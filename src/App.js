...
Version: v3.1.0 (Fix: Finance Sort & Balance Logic)
Updates:
- Fixed Firestore query index error (simplified orderBy).
- Changed Finance logic from Income/Expense to Account Balances.
----------------------------------------------------------------
*/
...
{ id: "finance", icon: PieChart, label: "財報" },
...
// --- New Feature: Financial Reports (財務財報 - Modified) ---
...
managementBalance: "", // 管理費帳戶餘額
publicFundBalance: "", // 公共基金帳戶餘額
summary: "",
link: ""
});

useEffect(() => {
// FIX: Simplified query to avoid index errors. Sorted in JS.
const q = query(
...
// JS Sort just to be safe if users back-date entries
docs.sort((a, b) => {
...
const handleAdd = async () => {
if (!newReport.managementBalance || !newReport.publicFundBalance) {
showToast("請輸入金額", "error");
return;
}

const mgmt = parseInt(newReport.managementBalance) || 0;
const publicFund = parseInt(newReport.publicFundBalance) || 0;
const totalAssets = mgmt + publicFund; // Calculate total assets
...
{
...newReport,
managementBalance: mgmt,
publicFundBalance: publicFund,
...
return new Intl.NumberFormat('zh-TW', { style: 'currency', currency: 'TWD', minimumFractionDigits: 0 }).format(num || 0);
};
...
<label style={{ fontSize: "12px", color: "#64748b" }}>1. 社區管理費帳戶餘額 ($)</label>
<input
type="number"
style={styles.input}
placeholder="0"
value={newReport.managementBalance}
onChange={(e) => setNewReport({ ...newReport, managementBalance: e.target.value })}
/>

<label style={{ fontSize: "12px", color: "#64748b" }}>2. 社區公共基金帳戶餘額 ($)</label>
<input
type="number"
style={styles.input}
placeholder="0"
value={newReport.publicFundBalance}
onChange={(e) => setNewReport({ ...newReport, publicFundBalance: e.target.value })}
/>
...
<div style={{ flex: 1, backgroundColor: "#F0F9FF", padding: "12px", borderRadius: "12px" }}>
<div style={{ fontSize: "12px", color: "#475569", display: "flex", alignItems: "center", gap: "4px" }}>
<Wallet size={12} color="#0284C7" /> 管理費帳戶
</div>
<div style={{ fontSize: "15px", fontWeight: "600", color: "#0284C7", marginTop: "4px" }}>
{formatCurrency(item.managementBalance)}
</div>
</div>
<div style={{ flex: 1, backgroundColor: "#FFF7ED", padding: "12px", borderRadius: "12px" }}>
<div style={{ fontSize: "12px", color: "#475569", display: "flex", alignItems: "center", gap: "4px" }}>
<Landmark size={12} color="#EA580C" /> 公共基金
</div>
<div style={{ fontSize: "15px", fontWeight: "600", color: "#EA580C", marginTop: "4px" }}>
{formatCurrency(item.publicFundBalance)}
</div>
</div>
</div>
...
<div>
<span style={{ fontSize: "12px", color: "#64748B" }}>社區總資產</span>
<div style={{ fontSize: "20px", fontWeight: "800", color: "#0F172A" }}>
{formatCurrency(item.totalAssets || (item.managementBalance + item.publicFundBalance))}
</div>
</div>
{item.link ? (
<a
...
>
Fengye Zheyin Community App v3.1.0
</p>
</div>
);
};
